...
    And table should contain the created category

  Scenario: TC_ADMIN_CAT_14 - Ensure the validation when adding the category without an name
    Given Admin is on Categories page
    When Admin saves category without name
    Then error message should be visible

  Scenario: TC_ADMIN_CAT_15 - Verify Admin user can Edit existing category successfully
  Given Admin is on Categories page
  When Admin adds a new category
  And Admin edits the created category to "Edited Cat"
  Then URL should be "/ui/categories"
  And table should contain "Edited Cat"
